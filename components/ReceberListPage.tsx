...
import { deleteReceber, listReceber, Receber } from '../core/receber';
...
export default function ReceberListPage() {
    ...
    const aplicar = useCallback(() => {
        ...
        if (term) {
            base = base.filter((it) =>
                `${it.clienteNome ?? ''} ${it.ficha ?? ''} ${it.cidade ?? ''} ${it.valorReceber} ${it.createdAt ?? it.vencimento}`
                    .toUpperCase()
                    .includes(term)
            );
        }
        if (tipoData === 'dia') {
            if (dataIni) base = base.filter((it) => (it.createdAt ?? it.vencimento)?.slice(0, 10) === dataIni);
        } else if (dataIni && dataFim) {
            base = base.filter((it) => {
                const d = (it.createdAt ?? it.vencimento)?.slice(0, 10);
                return d && d >= dataIni && d <= dataFim;
            });
        }
        setLista(base);
        setPagina(1);
            ...
            <main className="flex-grow mx-4">
                <div className="bg-white p-4 rounded-xl">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-xl lg:text-2xl font-bold text-[#17686f]">Contas a Receber</h2>
                    </div>

                    {/* 🔹 Filtros + Botões na mesma linha */}
                            ...
                            <table className="w-full table-fixed text-sm text-left border-collapse">
                                <thead className="bg-[#1c7d87] text-white">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3">Valor Pcl.</th>
                                    ...
                                    {visiveis.length === 0 ? (
                                        ...
                                        visiveis.map((cad) => (
                                            <tr key={cad.id} className="even:bg-[#c4f9ff]">
                                                <td className="px-4 py-3">{cad.status}</td>
                                                <td className="px-4 py-3">{cad.cobradorNome ?? '—'}</td>
                                                <td className="px-4 py-3">{cad.cidade ?? '—'}</td>
                                                <td className="px-4 py-3">{cad.clienteNome ?? '—'}</td>
                                                <td className="px-4 py-3">{cad.ficha ?? '—'}</td>
                                                <td className="px-4 py-3">{formatBRFromISO(cad.createdAt ?? cad.vencimento)}</td>
                            ...
                            {!loading && lista.length > 0 && (
                                <div className="mt-6 flex flex-col xs:flex-row justify-center items-center gap-3">
                                    ...
                                    <button
                                        ...
                                        onClick={() => setPagina((prev) => Math.min(prev + 1, totalPag))}
                                        className="px-4 py-2 rounded bg-[#007cb2] text-white hover:bg-[#00689c] disabled:bg-gray-400"
                                    >
                                        Próxima
                                    </button>
            ...
            {relatorioOpen && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-lg p-6 w-full max-w-lg">
                        <h3 className="text-lg font-semibold mb-4">Relatório de Contas a Receber</h3>
